...
def run_one_ticker(sym: str, csv_path: str, plot=True):
    ...
    test_dates = target_dates[-len(y_test):]

    if plot:
        plt.figure(figsize=(12, 5))
        plt.plot(test_dates, targs_real.ravel(), label='Actual')
        plt.plot(test_dates, preds_real.ravel(), label='Predicted')
        ...
        residuals = (targs_real - preds_real).ravel()
        plt.figure(figsize=(12, 4))
        plt.plot(test_dates, residuals, linewidth=1)
        plt.axhline(0, linestyle='--', linewidth=1)
        plt.title(f"{sym} Residuals over Time (Actual - Predicted)")
        plt.xlabel("Date");
        plt.ylabel("Residual")
        plt.tight_layout();
        plt.show()

        y_true_flat = targs_real.ravel()
        y_pred_flat = preds_real.ravel()
        plt.figure(figsize=(5.5, 5.5))
        plt.scatter(y_true_flat, y_pred_flat, s=8, alpha=0.6)
        lo = min(y_true_flat.min(), y_pred_flat.min())
        hi = max(y_true_flat.max(), y_pred_flat.max())
        plt.plot([lo, hi], [lo, hi], linestyle='--')
        plt.title(f"{sym} Actual vs Predicted")
        plt.xlabel("Actual");
