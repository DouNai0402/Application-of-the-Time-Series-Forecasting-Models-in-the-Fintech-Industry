...
# read data
SPY_DF = pd.read_csv("../Dataset/processed_data/SPY_processed.csv", index_col='Date', parse_dates=True)
...
def create_sequence(df, feature_cols, target_col,seq_length):
    ...
    X, y = [], []
    for  i  in range(len(features) - seq_length):   #len(features) is the number teh data
...
X_train, X_val, y_train, y_val = train_test_split(
    X_temp, y_temp, test_size = 0.1, shuffle=False
)
...
X_val_scaled = transform_X(X_val, feature_scaler)
...
model = LSTMModel(input_dim=len(FEATURES), hidden_dim=64, num_layers=3, dropout_rate=0.1).to(device)
loss_fn = nn.MSELoss()
...
for epoch in range(EPOCHS):
    ...
    with torch.no_grad():
        for X_val_batch, y_val_batch in train_loader:
            ...
            total_loss_v += val_loss.item() * X_val_batch.size(0)
    avg_loss_v = total_loss_v / BATCH_SIZE
    print(f"Epoch {epoch + 1}/{EPOCHS}: VAL Loss = {avg_loss_v:.6f}")
        # val_pred = model(X_train_t)
        # val_loss = loss_fn(val_pred, y_val_t)

    # print(f"Epoch {epoch + 1}: Train Loss = {loss.item():.6f}, Val Loss = {val_loss.item():.6f}")

    # check early stopping
    if avg_loss_v < best_val_loss:
        best_val_loss = avg_loss_v
        counter = 0
...
actual = np.vstack(all_targets)
    ...
    # actual = target_scaler.inverse_transform(y_test_t.numpy().reshape(-1, 1))
